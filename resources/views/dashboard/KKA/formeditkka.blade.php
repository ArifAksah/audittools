    ...
    <div class="layout-page">
        ...
        <div class="content-wrapper">
            ...
            <div class="container-xxl flex-grow-1 container-p-y">
                ...
                @endif
                <div class="card">
                    <div class="card-header">{{ __('Edit KKA') }}</div>

                    <div class="card-body">
                        <form method="POST" action="{{ route('kkas.update', $kka->id) }}" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" name="audit_kka" id="audit_kka" value="{{ $kka->audit_kka }}" class="form-control" readonly>
                            </div>

                            <!-- Select Auditor -->
                            <div class="mb-3">
                                <label for="auditor_kka" class="form-label">{{ __('Auditor') }}</label>
                                <input type="text" name="auditor_kka" id="auditor_kka" value="{{ $kka->auditor_kka }}" class="form-control" readonly>
                            </div>

                            <!-- Kondisi -->
                            <div class="mb-3">
                                <label for="kondisi_teks" class="form-label">{{ __('Kondisi') }}</label>
                                <textarea name="kondisi_teks" id="kondisi_teks" class="form-control" required>{{ $kka->kondisi_teks }}</textarea>
                            </div>
                            <!-- Kondisi Image -->
                            <div class="mb-3">
                                <label for="kondisi_images" class="form-label">{{ __('Kondisi Images') }}</label>
                                @foreach(json_decode($kka->kondisi_images, true) as $image)
                                    <img src="{{ asset('storage/' . $image) }}" alt="Kondisi Image" style="max-width: 200px; max-height: 200px; margin-bottom: 10px;">
                                @endforeach
                            ...
                            <div class="mb-3">
                                ...
                                <textarea name="kriteria_teks" id="kriteria_teks" class="form-control" required>{{ $kka->kriteria_teks }}</textarea>
                            </div>
                            <!-- Kriteria Image -->
                            <div class="mb-3">
                                <label for="kriteria_images" class="form-label">{{ __('Kriteria Images') }}</label>
                                @foreach(json_decode($kka->kriteria_images, true) as $image)
                                    <img src="{{ asset('storage/' . $image) }}" alt="Kriteria Image" style="max-width: 200px; max-height: 200px; margin-bottom: 10px;">
                                @endforeach
                            ...
                            <div class="mb-3">
                                ...
                                <textarea name="sebab" id="sebab" class="form-control" required>{{ $kka->sebab }}</textarea>
                            </div>
                            <!-- Akibat -->
                            <div class="mb-3">
                                <label for="akibat" class="form-label">{{ __('Akibat') }}</label>
                                <textarea name="akibat" id="akibat" class="form-control" required>{{ $kka->akibat }}</textarea>
                            </div>
                            <!-- Rekomendasi -->
                            <div class="mb-3">
                                <label for="rekomendasi" class="form-label">{{ __('Rekomendasi') }}</label>
                                <textarea name="rekomendasi" id="rekomendasi" class="form-control">{{ $kka->rekomendasi }}</textarea>
                            </div>
                            <!-- Evidence -->
                            <div class="mb-3">
                                <label for="evidence" class="form-label">{{ __('Evidence') }}</label>
                                <input type="text" name="evidence" id="evidence" class="form-control" value="{{ $kka->evidence }}">
                            </div>
